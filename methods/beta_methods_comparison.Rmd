...
There was some discussion about whether to use presence-absence metrics, or abundance-weighted metrics. For BBS, we don't have good estimates of abundance so typically people just use presences. However, for FIA, we can use basal area to estimate abundance of each species. I've presented both in this document. I think it is fine to use just presence-absence as long as we are consistent throughout. I will continue to calculate and make maps for both, and we can decide for each paper which one(s) to present. For the most part, they should yield similar results at the broad scales we are looking at.

# 5. Next steps: phylogenetic and functional diversity

These metrics are very cutting-edge but haven't been integrated with phylogenetic and functional beta-diversity methods that are easily available in R. For now, we are using older methods in the *vegan* and *picante* packages to get phylogenetic and functional beta-diversity. It would be nice to integrate these new methods with phylogenetic and functional diversity, but that would require writing a lot of our own code and getting the math right. It would be a nice addition to the project but would be a lot of work. If anyone is interested, let me know. If not, we can stick with the out-of-the-box methods that are already out there.
