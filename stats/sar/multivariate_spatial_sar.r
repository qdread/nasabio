...
# Yet another fork created to do SAR models
# QDR/Nasabioxgeo/29 April 2019

# Modified 30 Apr 2019: Get rid of the multivariate response since SAR only supports single variable response
...
source('/mnt/research/nasabio/code/fit_mv_sar.r')

# Fit the model for the given response variable, taxon, and ecoregion
options(mc.cores = 3)

if (taxon == 'bbs') {
  load('/mnt/research/nasabio/temp/bbs_spatial_mm_dat_50k_sar.RData')
  geodat <- bbsgeo
  biodat <- bbsbio
  distmat <- bbsdist
  siteid <- 'rteNo'
  ...
  load('/mnt/research/nasabio/temp/fia_spatial_mm_dat_50k_sar.RData')
  geodat <- fiageo
  biodat <- fiabio
  distmat <- fiadist
  siteid <- 'PLT_CN'
...
# Save all fits
save(fit, file = paste0('/mnt/research/nasabio/temp/mvspam/sarfit',task,'.RData'))
