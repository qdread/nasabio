...
kfold_rmse <- function(fit, k, n_chains, n_iter, n_warmup, delta = 0.8, seed = 101) {
  require(brms)
  require(purrr)
  require(dplyr)
...
kf <- kfold_rmse(fit$model, k = 5, n_chains = 2, n_iter = NI, n_warmup = NW, delta = delta, seed = task + 303)	
	
save(kf, file = paste0('/mnt/research/nasabio/temp/mvspam/kfold_', task, '.RData'))
