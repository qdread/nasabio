...
fiabio <- Reduce(left_join,
				 ...
				 ad %>% 
					filter(radius == 100) %>% 
					select(PLT_CN, richness, shannon) %>%
					rename(alpha_richness = richness, alpha_shannon = shannon),
				 bd %>%
					filter(radius == 100) %>%
				 ...
				 gd %>% 
					filter(radius == 100) %>% 
					select(PLT_CN, richness, shannon) %>%
					rename(gamma_richness = richness, gamma_shannon = shannon)))
					
write.csv(fiabio, file.path(fp, 'fia_bio_formixedmodels.csv'), row.names = FALSE)

# Load the geodiversity data frames that are needed: elevation, bio5k, geo and soil, dhi
...
fiageo <- Reduce(left_join,
				 ...
				 othermean %>%
					select(PLT_CN, dhi_gpp_5k_100_mean),
				 othersd %>% 
					select(PLT_CN, dhi_gpp_5k_100_sd, geological_age_5k_100_diversity_geodiv, soil_type_5k_100_diversity_geodiv) ))
