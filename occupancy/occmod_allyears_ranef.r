...
# Modification 25 August: get rid of the random effect
# Have to do this in jags because stan does not support the discrete parameter z
...
###Combine input data for chosen year: detection array
X <- bbs_arrays$x[[which(bbs_arrays$year == yr)]]

# Create input data list
sp.data <- list(nspec=dim(X)[3], nsite=dim(X)[1], nrep=dim(X)[2], X=X)
