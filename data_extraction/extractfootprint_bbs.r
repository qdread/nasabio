...
load('/mnt/research/nasabio/data/precalcdist/distlogical_bbs_hf.r')
...
for (j in rowidxmin:rowidxmax) {
	...
	extractBox(coords = with(bbsll, cbind(lon, lat))[j,,drop=FALSE],
		   ...
		   radius = 500,
		   fp = scratch_path,
		   filetags = paste('bbshf', row.names(bbsll)[j], sep = '_'))
	file_j <- paste0(scratch_path, '/bbox_bbshf_', row.names(bbsll)[j], '.tif')
...
save(stats_by_point, file = paste0('/mnt/research/nasabio/data/bbs/geostats/footprint_',slice,'.r'))
