...
for (j in rowidxmin:rowidxmax) {
	...
	extractBox(coords = with(bbsll, cbind(lon, lat))[j,,drop=FALSE],
		   ...
		   radius = 500,
		   fp = scratch_path,
		   filetags = paste('bbs5k', row.names(bbsll)[j], k, sep = '_'))
	file_j <- paste0(scratch_path, '/bbox_bbs5k_', row.names(bbsll)[j], '_', k, '.tif')
...
save(stats_by_point, file = paste0('/mnt/research/nasabio/data/bbs/climstats/biocloud5k_',slice,'.r'))
