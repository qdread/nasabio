...
metric <- as.numeric(Sys.getenv('PBS_ARRAYID'))
...
print(metric)
load(paste0('/mnt/research/nasabio/data/fia/diversity/usa/metric_beta_', metric, '.r'))


# For each year and route number, get the median beta diversity within each radius.
# The pairwise table was constructed to only go up to 200 km, so that is all we will have.
radii <- c(5, 10, 20, 50, 75, 100, 150, 200) # in km
...
write.csv(fia_beta, file = paste0('/mnt/research/nasabio/data/fia/diversity/filtered/beta_metric_', metric, '.csv'), row.names = FALSE)

#############################################
# Combine all metrics into one csv

library(dplyr)
beta_metrics <- lapply(1:21, function(metric) read.csv(paste0('/mnt/research/nasabio/data/fia/diversity/filtered/beta_metric_', metric, '.csv'), stringsAsFactors = FALSE))
