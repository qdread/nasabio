...
neighbordivfromtable <- function(x) {
	...
	for (i in 1:length(radii)) {
		neighbors_incircle <- beta_div_list[[x$n - rowidxmin + 1]][neighbordists <= radii[i], , drop = FALSE]
...
write.csv(fia_beta, file = paste0('/mnt/research/nasabio/data/fia/diversity/usa/tdbeta_averages_', slice, '.csv'), row.names = FALSE)
...
for (i in 1:250) {
	fia_beta_list[[i]] <- read.csv(paste0('/mnt/research/nasabio/data/fia/diversity/usa/tdbeta_averages_', i, '.csv'), stringsAsFactors = FALSE)
}

fia_beta_list <- do.call('rbind', fia_beta_list)

write.csv(fia_beta_list, file = '/mnt/research/nasabio/data/fia/fiausa_betatd.csv', row.names = FALSE)