...
#########################################################################################
# amendment on 17 aug: only go up to 100 km and get more slices

### amendment on 31 july: for only the smaller radii right now.
radii <- radii[1:8]

nslices <- c(rep(250, 9), 2000, 2000)
...
write.csv(fia_betapart, file = '/mnt/research/nasabio/data/fia/fia_betapart_to100.csv', row.names = FALSE)
