...
fixedbbsmat_nonzero <- fixedbbsmat[, ns > 0 & !(sppids %in% nocturnalbirdAOUs) & !(sppids %in% c('3089','4451'))]
sppids_nonzero <- sppids[ns > 0 & !(sppids %in% nocturnalbirdAOUs) & !(sppids %in% c('3089','4451'))]
...
save(fd_all, zerocols, file = file.path(fp, paste0('birdfuncdivobjectpresence', task, '.r')))
