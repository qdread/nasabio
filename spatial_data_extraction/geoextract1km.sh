...
cp /mnt/research/nasabio/code/master_extract_1kmonly.r $TMPDIR/rcode.r
cd $TMPDIR
now="$(date +'%d%h%Y%H%M')"

# Cobble together the R command using the environmental variables supplied.
cmd="R CMD BATCH --no-save --no-restore '--args taxon=\""$taxon"\" geovar=\""$geovar"\"' rcode.r /mnt/ffs17/groups/nasabio/routputfiles/"$taxon"_"$geovar"_"$PBS_ARRAYID".txt"
eval $cmd
