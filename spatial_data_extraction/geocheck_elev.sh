...
cd /mnt/research/nasabio/data/${taxon}/elevstats_usa
...
for j in $(seq 0 $nvars); do
	...
	for i in $(seq 1 ${totaljobs[j]}); do
		...
		if [ "$jobsleft" -eq "250" ] || ([ "$i" -eq "${totaljobs[j]}" ] && [ "$jobsleft" -gt "0" ]); then
			# Create qsub string
			qsub_string="qsub elevextract.sh -N ${taxon}_${geovars[j]} -v taxon=${taxon},geovar=${geovars[j]} -l walltime=${walltime},mem=1gb -t ${needtorun%?}"
			# Paste qsub string at the bottom of qsub file
			echo $qsub_string >> /mnt/research/nasabio/code/elev_qsub_all.txt
