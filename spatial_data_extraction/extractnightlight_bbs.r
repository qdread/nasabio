...
load('/mnt/research/nasabio/data/precalcdist/distlogical_bbs_nightlight.r')
...
for (j in rowidxmin:rowidxmax) {
	...
	extractBox(coords = with(bbsll, cbind(lon, lat))[j,,drop=FALSE],
		   ...
		   radius = 500,
		   fp = scratch_path,
		   filetags = paste('bbsnight', row.names(bbsll)[j], sep = '_'))
	file_j <- paste0(scratch_path, '/bbox_bbsnight_', row.names(bbsll)[j], '.tif')
...
save(stats_by_point, file = paste0('/mnt/research/nasabio/data/bbs/geostats/nightlight_',slice,'.r'))
