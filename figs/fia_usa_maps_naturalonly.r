...
# Additional version 01 Mar: Only "natural" or non-plantation plots.
...
ad <- read.csv(file.path(fp, 'fiausa_natural_alpha.csv')) %>% left_join(fiacoords)
bd <- read.csv(file.path(fp, 'fiausa_natural_betatd.csv')) %>% left_join(fiacoords)
gd <- read.csv(file.path(fp, 'fiausa_natural_gamma.csv')) %>% left_join(fiacoords)
...
colscalebeta <- scale_colour_gradientn(name = 'Taxonomic\nbeta-diversity', colours = colorRampPalette(colors=RColorBrewer::brewer.pal(9, 'Purples'), bias = 0.5)(9))
colscalealpha <- scale_colour_gradientn(name = 'Taxonomic\nalpha-diversity', colours = colorRampPalette(colors=RColorBrewer::brewer.pal(9, 'Purples'), bias = 2)(9))
colscalegamma <- scale_colour_gradientn(name = 'Taxonomic\ngamma-diversity', colours = rev(colorRampPalette(colors=RColorBrewer::brewer.pal(9, 'RdYlBu'), bias = 1)(9)))
...
fpfig <- 'C:/Users/Q/google_drive/NASABiodiversityWG/Figures/fia_diversity_maps/removed_plantations'
