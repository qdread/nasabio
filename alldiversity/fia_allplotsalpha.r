...
# Calculate alpha and gamma diversity within different radii.
...
alpha_div <- array(NA, dim = c(nrow(fiaplotmat), length(radii), 17)) # needs to have one for each of the types of diversity.
...
for (p in rowidxmin:rowidxmax) {
	...
	for (r in 1:length(radii)) {
		...
		alpha_div[p, r, ] <- diversity_3ways(m = neighbs, flavor = 'alpha', 
...
dimnames(alpha_div) <- list(cnames, paste(r,radii,sep='_'), NULL)
