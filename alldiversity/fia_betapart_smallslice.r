...
# Edit 18 Aug: only the desired radii, and using Baselga/Sorensen/binary only (with beta_part)
# This will eventually enable us to incorporate taxonomic information.
task <- as.numeric(Sys.getenv('PBS_ARRAYID'))
radii <- c(5, 10, 20, 50, 100) * 1000
n_slices <- 250
...
null_result <- 	res <- data.frame(family = rep(c('podani', 'baselga'), each = 10),
					  ...
					  abundance = rep(c(FALSE,TRUE), each = 20),
					  beta = NA)
...
for (p in 1:length(idxmin:idxmax)) {
	...
	if(inherits(mat_p, 'matrix')) {
        ...
        beta_td <- beta_baselga_podani(m = mat_p, abundance = TRUE)
...
write.csv(fia_betadiv, file = paste0('/mnt/research/nasabio/data/fia/betaoutput/sliced2/betabaselga_',as.character(as.integer(r)),'_',slice,'.csv'), row.names = FALSE)						  
