...
rowidx <- round(seq(0,nrow(fixedbbsmat_byroute),length.out=n_slices + 1))
rowidxmin <- rowidx[slice]+1
rowidxmax <- rowidx[slice+1]

alpha_div <- diversity_3ways(m = fixedbbsmat_byroute[rowidxmin:rowidxmax,], flavor = 'alpha', 
                             dotd=T, dopd=T, dofd=T, abundance=F,
                             pddist = ericdist, fddist = birdtraitdist,
                             nnull = nnull,
                             phylo_spp = NULL, func_problem_spp = NULL, combine = F)

alpha_div <- cbind(bbscov[rowidxmin:rowidxmax,], alpha_div)
save(alpha_div, file = paste0('/mnt/research/nasabio/data/bbs/diversity/alpha_', slice, '.r'))